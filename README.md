...
`Docker-Desktop` и `docker` потребляют ресурсы системы, если есть запущенные контейнеры. Поэтому стоит убедиться, что `Docker-Desktop` выключен, когда он вам не нужен, или нет активных, вручную запущенных контейнеров.
...
Для удобства (чтобы рабочей директорией терминала был ваш репозиторий) можно добавить в конец `~/.bashrc` строку `cd ~/<students-private-repo-name>`, например `cd ~/hse_ivivivanov`, или любой другой предпочтительный путь. 
